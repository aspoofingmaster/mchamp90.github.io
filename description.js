...
$(function() {
  $("li").on("click",function() {
	  if(this.id=="dnt") {
		  $("#dnt_txt").html("You can donate USD via PayPal mail: mchamp23"+"@"+"gmail.com");
	  }
  });
});
	...
	$.ajax({
		...
		success: function (returnhtml) {
			...
			for (var dicNow in decodeResp) {
				...
				htmlnews +=  "<li><a href='"+urlOpen+"' target='_blank'><img class='icon' src='theme.png'/><label>"+decodeResp[dicNow].name+" v"+decodeResp[dicNow].version+"</label></a></li>";
			}
